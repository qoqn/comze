...
function hasDevSuffix(version: string): boolean {
    ...
    return lower.includes('-dev') || lower.includes('alpha') || lower.includes('beta') ||
        lower.includes('-rc') || lower.includes('@dev') || lower.includes('@alpha') ||
        lower.includes('@beta') || lower.includes('@rc');
}
...
export function normalizeVersionString(version: string): string {
    return version.trim().replace(/^v/i, '').replace(/@(dev|alpha|beta|rc|stable)$/i, '');
}
...
export function isDevVersion(version: string): boolean {
    const lower = version.toLowerCase();
    return lower.startsWith('dev-') || lower.endsWith('-dev') || lower.endsWith('.x-dev') ||
        lower.includes(' as ') || hasDevSuffix(version);
}
